...
from defectio.internal import attr_extensions
from defectio.internal import data_binding
from defectio.internal import enums
...
class BasicAuthHeader:
    ...
    charset: str = attr.field(
        default="utf-8", validator=attr.validators.instance_of(str)
    )
...
class ProxySettings:
    ...
    def _(
        self,
        _: attr.Attribute[typing.Union[None, str, yarl.URL]],
        value: typing.Union[None, str, yarl.URL],
    ) -> None:
        if value is not None and not isinstance(value, (str, yarl.URL)):
            raise ValueError(
                "ProxySettings.url must be None, a str, or a yarl.URL instance"
            )

    trust_env: bool = attr.field(
...
class HTTPTimeoutSettings:
    ...
    def _(
        self,
        attrib: attr.Attribute[typing.Optional[float]],
        ...
        if value is not None and (not isinstance(value, (float, int)) or value <= 0):
            raise ValueError(
                f"HTTPTimeoutSettings.{attrib.name} must be None, or a POSITIVE float/int"
            )
...
class HTTPSettings:
    ...
    enable_cleanup_closed: bool = attr.field(
    ...
    force_close_transports: bool = attr.field(
        default=True, validator=attr.validators.instance_of(bool)
    )
    ...
    def _(
        self, _: attr.Attribute[typing.Optional[int]], value: typing.Optional[int]
    ) -> None:
        # This error won't occur until some time in the future where it will be annoying to
        # try and determine the root cause, so validate it NOW.
        if value is not None and (not isinstance(value, int) or value <= 0):
            raise ValueError(
                "http_settings.max_redirects must be None or a POSITIVE integer"
            )
    ...
    timeouts: HTTPTimeoutSettings = attr.field(
        factory=HTTPTimeoutSettings,
        validator=attr.validators.instance_of(HTTPTimeoutSettings),
    )
    """Settings to control HTTP request timeouts.
